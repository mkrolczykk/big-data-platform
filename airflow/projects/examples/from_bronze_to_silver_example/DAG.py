...
DAG_name = 'from_bronze_to_silver_example_dag'
...
def prepare_input(**kwargs):
    ...
    # tmp source (just for presentation):
    aws_input_uri = f's3a://{s3_bucket_name}/01_bronze/car_sales_data_processed/'
    ...
    # tmp output (just for presentation):
    ti.xcom_push(key='s3_output_uri', value=f's3a://{s3_bucket_name}/02_silver/car_sales_data_enriched/')
...
def process_from_bronze_to_silver_example_dag():
    ...
    run_spark_submit_task = SparkSubmitOperator(
        ...
        application="spark/jobs/sample-project/sample-project-pipeline/python/from_bronze_to_silver_example_job.py",
        application_args=[
...
main_dag = process_from_bronze_to_silver_example_dag()
DAGS = [main_dag]